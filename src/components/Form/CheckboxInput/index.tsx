...
export const AuthCheckbox = React.forwardRef<any, CheckBoxProps>(
  (props: CheckBoxProps, ref) => {
    const { checked, onClick, error, labelSize, ...checkinputProps } = props;
    ...
    return (
      <CheckboxInput>
        <Input
          ...
          ref={ref}
        />
        <CheckboxDisplay onClick={onClick ? onClick : () => {}}>
          {checked && <Icon iconName="check" color="WHITE" size="D16" />}
        </CheckboxDisplay>
        <Label size={labelSize}>
          By clicking Sign up, you are agreeing to our{' '}
          <TextLink to={LINKS.termsOfService}>Terms of Use</TextLink> and{' '}
          <TextLink to={LINKS.privacyPolicy}>Privacy Policy</TextLink>
        </Label>
